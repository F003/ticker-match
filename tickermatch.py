...
def run():
    ...
    # using pandas as pd and astype(str) method change 'CompanyName' and 'Ticker' columns from df_db to strings
    ...
    display_message("Merging names with tickers...\n")
    ...
    # drop CompanyName_y column from df_merged
    df_merged.drop(columns=['CompanyName_y'], inplace=True)

    # rename 'CompanyName_x' column to 'CompanyName'
    df_merged.rename(columns={'CompanyName_x': 'CompanyName'}, inplace=True)

    # Make SimilarityScore 4th column and Ticker 3rd column in df_merged
